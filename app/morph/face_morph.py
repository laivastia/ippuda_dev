...
def generate_morph_sequence(duration,frame_rate,img1,img2,points1,points2,tri_list,size,dir1,dir2):
    ...
    dims = (512, 512)
    fps = 20
    output= r'C:\tmp\video_output.mp4'
    ...
    for j in range(0, num_images):
        ...
        src_img_buffer1 = BytesIO()
        res.save(src_img_buffer1, format='JPEG')
        img_src_binary1 = Binary(src_img_buffer1.getvalue())

        # Create a dictionary object to hold the metadata and the binary data of the image
        src_image_metadata1 = {'filename': 'sequence_'+ str(round(j))+'.jpg', 'format': 'JPEG'}
        src_image_data1 = {'metadata': src_image_metadata1, 'image': img_src_binary1}
        # Insert the image data into the MongoDB collection
        dir1.insert_one(src_image_data1)

        src_img_buffer2 = BytesIO()
        res_orgin.save(src_img_buffer2, format='JPEG')
        img_src_binary2 = Binary(src_img_buffer2.getvalue())

        # Create a dictionary object to hold the metadata and the binary data of the image
        src_image_metadata2 = {'filename': 'sequence_'+ str(round(j))+'.jpg', 'format': 'JPEG'}
        src_image_data2 = {'metadata': src_image_metadata2, 'image': img_src_binary2}
        # Insert the image data into the MongoDB collection
        dir2.insert_one(src_image_data2)
